document.addEventListener('DOMContentLoaded', function(){
    ...
    let slideList = document.querySelector('.slider-item');

    btnSlideRight.addEventListener('click', () => {
      for (const key in slideList) {
        if (Object.hasOwnProperty.call(slideList, key)) {
          const element = slideList[key];
          if(element.classList.value === "slider-item active")
          {
            element.classList.remove('active');
            element.nextElementSibling.classList.add('animation-rtl');
          }
        }
      }
    });
