document.addEventListener('DOMContentLoaded', function(){
    ...
    //auto Slider
    let btnPrev = document.querySelector('#btnSlideLeft');
    let btnNext = document.querySelector('#btnSlideRight');
    let slideList = document.querySelector('.slider-list');
    let slideItem = document.querySelector('.slider-item');
    let slideItemAll = document.querySelectorAll('.slider-item').
    let slideItemAll = document.querySelectorAll('.slider-item').length;
    let slideItemWidth = slideItem.offsetWidth + 10;
    let slideItemWidthMax = (slideItemAll-1)*(slideItemWidth + 10);
    let vitri = 0;
    const duration = 6000;

    let nextSlide =  () => {
      vitri = vitri - slideItemWidth;
      if(vitri >  -slideItemWidthMax){
        slideList.style.transform = "translateX("+vitri+"px)";
      }else {
        vitri = 0;
        slideList.style.transform = "translateX("+vitri+"px)";
      }
    ...
    btnPrev.onclick = () => slide("prev");
    btnNext.onclick = () => slide("next");
      ...
      for (const key in paginationAll) {
        if (Object.hasOwnProperty.call(paginationAll, key)) {
          paginationAll[key].onclick = () => {
            for (const index in paginationAll) {
              if (Object.hasOwnProperty.call(paginationAll, index)) {
                paginationAll[index].classList.remove("active");
              }
            }
            vitri = -(key*slideItemWidth);
